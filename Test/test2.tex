...
stranieri}, pari al \textbf{9,2\%} della popolazione totale.
L'\textbf{età media} è di \textbf{37,0 anni}, nettamente inferiore a
quella italiana (47,8), a conferma di una popolazione giovane e in età
lavorativa. Il \textbf{58,3\%} risiede al Nord, con forte presenza in
Emilia-Romagna.
\end{tcolorbox}
...
sempre da etichette testuali. La piattaforma di acquisto offre l'interfaccia in inglese solo dopo il login, rendendola inaccessibile proprio quando l'utente straniero ne ha piu' bisogno: durante la registrazione e il primo accesso. È inoltre utile
generare un riepilogo stampabile dei passi completati, così che
l'utente possa portare a casa un riferimento autonomo tra una
sessione e l'altra. Queste scelte, progettate per il caso più
estremo, migliorano l'esperienza per tutti gli utenti fragili.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=backgroundColor,colframe=borderColor,arc=2mm]
    ...
    \textbf{Obiettivo:} Rinnovare l'abbonamento mensile impersonale TPER in autonomia, senza assistenza. \\[4pt]
    ...
    \begin{itemize}
        ...
        \item Nessun percorso guest per un rinnovo di abbonamento impersonale: il sistema richiede registrazione anche per un prodotto che, per definizione, non è associato a una persona.
...
\begin{tcolorbox}[colback=backgroundColor,colframe=borderColor,arc=2mm]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item Il problema centrale \e che un rinnovo di abbonamento impersonale (che non richiede identificazione) viene trattato come un'operazione che necessita di account e registrazione. La richiesta di registrazione per un prodotto che per definizione non \e nominale genera un attrito procedurale sproporzionato rispetto alla natura dell'operazione (cfr. D5).
...
% ================================================================
% USE CASE #1: «Il codice sconosciuto»
% ================================================================

\begin{tcolorbox}[colback=primaryColor!10,colframe=primaryColor,arc=2mm]
    \textbf{Descrizione del Use Case \#1: Acquisto dell'abbonamento bloccato dal campo `Codice Fiscale''}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=backgroundColor,colframe=borderColor,arc=2mm]
    \textbf{Narrative del Use Case} \\[4pt]
    {\small
    \textbf{Scenario:} David ha deciso di comprare un abbonamento mensile. Sa che esiste la versione impersonale (non serve la foto) e gli basta per i tre mesi che deve restare a Bologna. Un collega italiano gli ha spiegato a grandi linee come fare: `vai sul sito tper.it, cerca abbonamento mensile impersonale e compra''. Apre il portatile, digita \texttt{tper.it} e inizia. \\[4pt]

    \textbf{Obiettivo:} Acquistare un abbonamento mensile impersonale TPER online, in autonomia completa. \\[4pt]

    \textbf{Sequenza delle azioni:}
    \begin{enumerate}
        \item David naviga su \texttt{tper.it} e, dopo qualche tentativo tra menu e sottomenu, arriva alla pagina `Abbonamenti''.
        \item Individua `Mensile Impersonale'' (l'etichetta `Impersonale'' non gli \e del tutto chiara, ma capisce dal prezzo e dal contesto che \e l'opzione giusta). Clicca `Acquista''.
        \item Viene reindirizzato a \texttt{solweb.tper.it}. La pagina mostra il riepilogo del prodotto e un pulsante `Procedi all'acquisto''. Clicca.
        \item Il sistema apre un modulo di pagamento. L'interfaccia \e in italiano. I campi sono: Nome, Cognome, Email, \textbf{Codice Fiscale}, Dati di pagamento.
        \item David compila Nome, Cognome, Email senza problemi. Arriva al campo `Codice Fiscale''. Non sa cosa sia. Il campo \e obbligatorio (marcato con asterisco rosso). Non c'\e alcun testo esplicativo, n\'e in italiano n\'e in inglese.
        \item Prova a inserire il suo numero di passaporto britannico. Il sistema rifiuta: `Il codice fiscale inserito non \e valido. Deve essere di 16 caratteri alfanumerici''.
        \item Cerca un pulsante di aiuto o un'icona `?'' accanto al campo. Non c'\e. Prova a cliccare sul nome del campo per vedere se compare un tooltip. Niente.
        \item A questo punto David \e bloccato. Ha tre opzioni: chiedere a un collega italiano (che gli spiegher\ cos'\e il CF e dove trovarlo), abbandonare, o cercare `Codice Fiscale'' su Google. Sceglie di scrivere al collega su WhatsApp.
        \item Il collega gli spiega che il Codice Fiscale \e un identificatore fiscale italiano di 16 caratteri, e che se non ce l'ha pu\o provare con il passaporto. David riprova con il passaporto: rifiutato di nuovo. Alla fine il collega gli dice di lasciar perdere e di comprare un biglietto singolo in edicola.
        \item David chiude la pagina e torna ai biglietti singoli. Ha perso 30 minuti.
    \end{enumerate}

    \textbf{Barriere previste:}
    \begin{itemize}
        \item \textbf{Assenza di spiegazione contestuale:} Il campo `Codice Fiscale'' \e obbligatorio ma non ha alcun tooltip, icona informativa o testo esplicativo che spieghi cosa sia e come si trova.
        \item \textbf{Multilingua assente:} L'intero flusso di acquisto \e solo in italiano. Per un utente con competenza linguistica 2/5, anche etichette semplici richiedono sforzo di decodifica e traduzione.
        \item \textbf{Nessun fallback per stranieri:} Il sistema non prevede un'alternativa al Codice Fiscale per cittadini stranieri (es. numero passaporto + data di nascita), bloccando completamente chi non ha un CF italiano.
        \item \textbf{Messaggio di errore non orientativo:} Il sistema rileva che il valore inserito non \e un CF valido, ma non spiega cos'\e un CF n\'e come ottenerlo, limitandosi a un messaggio puramente formale.
    \end{itemize}

    \textbf{Success criteria (non raggiunti):}
    \begin{itemize}
        \item David non completa l'acquisto: il flusso si interrompe al campo CF.
        \item David \e costretto a chiedere aiuto a un intermediario (collega italiano) per comprendere un termine di dominio, violando l'autonomia che un utente con competenza tecnica 4/5 si aspetterebbe.
        \item David torna al metodo di acquisto tradizionale (biglietto singolo in edicola), spendendo pi\u denaro ma evitando la frizione burocratica.
    \end{itemize}

    \textbf{Stima dei tempi:} 30 minuti prima dell'abbandono.
    }
\end{tcolorbox}
...
% ================================================================
% USE CASE #2: �La ricerca che non parla inglese�
% ================================================================

\begin{tcolorbox}[colback=primaryColor!10,colframe=primaryColor,arc=2mm]
    \textbf{Descrizione del Use Case \#2: Ricerca di informazioni su abbonamenti e agevolazioni --- barriera terminologica e motore di ricerca assente}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=backgroundColor,colframe=borderColor,arc=2mm]
    \textbf{Narrative del Use Case} \\[4pt]
    {\small
    \textbf{Scenario:} David ha saputo da un collega expat che esiste un abbonamento agevolato per chi ha reddito basso. Non sa se la sua situazione (lavoratore temporaneo, reddito medio) gli d\ diritto a sconti, ma vuole verificare. Apre \texttt{tper.it} e inizia a cercare. \\[4pt]

    \textbf{Obiettivo:} Trovare le informazioni sulle tipologie di abbonamento disponibili e capire se esistono agevolazioni tariffarie per il suo caso specifico. \\[4pt]

    \textbf{Sequenza:}
    \begin{enumerate}
        \item David digita `discount'' nella barra di ricerca interna del sito. Il sistema restituisce zero risultati. Prova `sconti''. I risultati sono comunicati stampa e notizie aziendali, nessuno pertinente.
        \item Prova `agevolazioni''. I risultati mostrano titoli come `Carta Agevolata'', `ISEE'', `DSU''. Per David, madrelingua inglese con competenza linguistica 2/5, questi termini sono privi di significato.
        \item Prova a navigare il menu principale: `Abbonamenti'' $\rightarrow$ `Agevolazioni''. Arriva a una pagina densa di testo italiano che cita `delibera regionale n. 2/2024'', `requisiti ISEE'', `soglie di reddito DSU''. Il testo \e pensato per un pubblico che conosce gi\ il sistema burocratico italiano.
        \item Cerca un pulsante `English'' o un'icona bandiera per cambiare lingua. Non c'\e.
        \item Prova a selezionare un paragrafo e tradurlo con Google Translate. La traduzione automatica rende il testo ancora pi\u confuso: `ISEE'' diventa `ISEE'' (non tradotto), `DSU'' rimane `DSU''.
        \item Dopo 10 minuti di tentativi, abbandona. Il giorno dopo chiede a un collega italiano, che gli spiega che con il suo reddito probabilmente non ha diritto ad agevolazioni. David si fida, ma nota che sul sito non c'\e un modo semplice per arrivare a questa conclusione da solo.
    \end{enumerate}

    \textbf{Barriere previste:}
    \begin{itemize}
        \item \textbf{Motore di ricerca interno non funzionale:} La ricerca non restituisce risultati per intenti reali dell'utente (`discount'', `sconti''), ma solo per termini burocratici esatti (`agevolazioni'', `DSU'').
        \item \textbf{Terminologia opaca per chi non conosce il dominio:} `ISEE'', `DSU'', `Carta Agevolata'' sono usati come etichette senza alcuna spiegazione contestuale o tooltip informativo.
        \item \textbf{Assenza totale di supporto multilingua:} Nessuna pagina del sito, n\'e informativa n\'e transazionale, \e disponibile in inglese o offre una traduzione automatica integrata.
        \item \textbf{Contenuti scritti per esperti del sistema italiano:} Il tono burocratico e i riferimenti normativi presuppongono una familiarit\ con il contesto istituzionale italiano che un residente temporaneo non possiede.
    \end{itemize}

    \textbf{Success criteria (non raggiunti):}
    \begin{itemize}
        \item David non trova le informazioni in autonomia: deve ricorrere a un intermediario (collega italiano).
        \item David non capisce se ha diritto o meno alle agevolazioni basandosi esclusivamente sul sito: la risposta arriva dal passa-parola informale tra expat.
        \item La ricerca interna si rivela inutilizzabile per query in inglese o per termini non esattamente burocratici.
    \end{itemize}

    \textbf{Stima dei tempi:} 10 minuti di navigazione, 5 minuti di spiegazione del collega.
    }
\end{tcolorbox}
\newpage
...
\begin{tcolorbox}[colback=backgroundColor,colframe=borderColor,arc=2mm]
    ...
    \begin{itemize}[leftmargin=*]
        \item \textbf{Terminologia tecnica non spiegata per l'utente non italiano (cfr. D3):} Etichette come `Mi Muovo'', `Boost'', `Impersonale'', `Codice Fiscale'', `ISEE'', `DSU'' vengono utilizzate senza alcuna spiegazione contestuale. Per un utente con competenza linguistica 2/5 e competenza di dominio 2/5, questi termini non sono autoesplicativi e costituiscono un blocco alla prosecuzione del flusso.
        \item \textbf{Assenza di supporto multilingua nei flussi transazionali (cfr. R12):} L'intero ecosistema TPER \e disponibile solo in italiano. Non esiste una versione inglese (n\'e di altre lingue) per le pagine di acquisto, rinnovo o informazione tariffaria. Un utente straniero con competenza tecnica alta \e comunque bloccato dalla barriera linguistica in fase di checkout.
...
\begin{tcolorbox}[colback=backgroundColor,colframe=borderColor,arc=2mm]
    {\small
    \textbf{Impatto del Use Case \#1 (Il codice sconosciuto) sul design:} \\[4pt]
    \begin{itemize}[leftmargin=*]
        \item Il campo `Codice Fiscale'' emerge come barriera critica perch\'e manca di qualsiasi ausilio contestuale. Un campo obbligatorio che richiede un identificatore specifico del sistema italiano, senza tooltip, senza esempio, senza fallback per stranieri, blocca completamente l'utente non italiano in fase di checkout (cfr. D3).
        \item Il problema non \e solo linguistico ma sistemico: il sistema richiede un dato che uno straniero temporaneo potrebbe non possedere, e non offre percorsi alternativi (passaporto, numero identificativo nazionale). L'assenza di un fallback trasforma un disallineamento burocratico in un blocco operativo (cfr. R10).
        \item La mancanza di supporto multilingua nel modulo di pagamento costringe David a chiedere aiuto a un intermediario per interpretare il campo, nonostante la sua competenza tecnica sia 4/5. Questo dimostra che l'autonomia dell'utente \e limitata non dalle sue capacit\ tecnologiche ma dalla barriera linguistica e terminologica dell'interfaccia (cfr. R12).
    \end{itemize}
    \vspace{0.2cm}
    \textbf{Impatto del Use Case \#2 (La ricerca che non parla inglese) sul design:} \\[4pt]
    \begin{itemize}[leftmargin=*]
        \item L'incapacit\ del motore di ricerca interno di interpretare query in inglese o query basate su bisogni (`discount'' invece di `agevolazioni'') rende il sito inutilizzabile per l'utente straniero in fase esplorativa. Un sistema di ricerca dovrebbe supportare sinonimi e intenti, non solo corrispondenze letterali con termini burocratici (cfr. R2).
        \item La terminologia opaca (ISEE, DSU, Carta Agevolata) senza spiegazioni contestuali crea un muro informativo: David non pu\o nemmeno formulare una domanda precisa perch\'e non conosce i termini del dominio. Serve un glossario contestuale o un sistema a domande guidate che traduca i bisogni dell'utente in termini di sistema (cfr. R13).
        \item L'assenza di una versione inglese delle pagini informative costringe l'utente a un lavoro di traduzione manuale (Google Translate) che degrada ulteriormente la qualit\ dell'informazione. La localizzazione non dovrebbe essere un'opzione ma una necessit\ imprescindibile per un servizio pubblico che si rivolge a una comunit\ multiculturale (cfr. R12).
    \end{itemize}
    }
\end{tcolorbox}
n{center}
