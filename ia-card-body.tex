...
\begin{enumerate}[leftmargin=*]
 ...
 \item \textbf{Conferma dai test utente (Phase A) e dalle personas (Phase B)}: Tutti i soggetti testati hanno evidenziato difficoltà strutturali, poi formalizzate nelle quattro personas del progetto:
 \begin{itemize}
   \item \textbf{Maria} (79 anni, SUS 27.5, competenza digitale minima): ``Non so neanche da dove cominciare'' --- arrivata su \texttt{tper.it}, non ha trovato un modo per acquistare e ha abbandonato. La sua bassissima competenza tecnica (C\&A: 1/5) richiede una struttura piatta con icone grandi e labeling in linguaggio quotidiano.
   \item \textbf{Laura} (70 anni, 7 errori in fase di test): ``L'interfaccia è troppo ingombrante, c'è troppa confusione'' --- mescola news aziendali e informazioni utili. La sua competenza di dominio medio-bassa richiede una separazione netta tra contenuti operativi e corporate.
   \item \textbf{Chris} (34 anni, 226 secondi per il Task 1): Ha riconosciuto che \texttt{tper.it} è solo informativo, ma ha dovuto indovinare l'esistenza di \texttt{solweb.tper.it} e Roger. La sua competenza linguistica media (madrelingua inglese) richiede supporto multilingua e labeling internazionale.
   \item \textbf{Fatima} (45 anni, competenza linguistica bassa in italiano): Parlante araba, necessita di navigazione in lingua araba con supporto RTL e icone visive per superare la barriera linguistica (C\&A linguistica: 2/5).
 \end{itemize}
...
\begin{itemize}
 \item \textbf{``Dove si compra?'' è la domanda più frequente (UR-01 -- Frammentazione canali)}: I test dimostrano che la maggior parte degli utenti arriva su \texttt{tper.it} con l'intenzione di acquistare, ma non trova un percorso chiaro. L'architettura deve rispondere a questa domanda \textbf{anche se} l'acquisto non avviene su \texttt{tper.it}. Lo use case UR-01 documenta come la frammentazione tra i cinque canali dell'ecosistema TPER (\texttt{tper.it}, \texttt{solweb.tper.it}, Roger, Punti Tper, tabaccherie, contactless) generi abbandono e frustrazione.
 ...
 \item \textbf{Canale suggerito per ogni esigenza}: Ogni tipologia di titolo di viaggio deve essere accompagnata dall'indicazione esplicita di dove acquistarla (\texttt{solweb}, Roger, Punto Tper, tabaccheria, contactless). Questo risponde allo use case \textbf{UR-02} (Confusione post-acquisto) e \textbf{UR-03} (Errori canale sbagliato), prevenendo l'abbandono per frustrazione.
\end{itemize}
...
\texttt{[Home]} \;--\; \texttt{[Pianifica]} \;--\; \texttt{[Dove Comprare]} \;--\; \texttt{[Biglietti]} \;--\; \texttt{[Abbonamenti]} \;--\; \texttt{[Aiuto]}
\end{tcolorbox}
\end{center}

\begin{enumerate}[leftmargin=*]
 \item \textbf{Home} (atterraggio): Hero section con route planner come funzionalità principale, seguita dalle card di accesso rapido alle sezioni e dal banner informativo ``tper.it è un sito informativo''.
 ...
 \item \textbf{Aiuto} (supporto): FAQ, guide passo-passo per ogni canale, contatti, trainer-mode per intermediari con toggle di attivazione, mappa dei Punti Tper.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
 ...
 \begin{tcolorbox}[colback=primaryColor!5,colframe=primaryColor,arc=2mm,width=0.95\textwidth]
 \small
 \texttt{[Home]} \quad \texttt{[Pianifica]} \quad \texttt{[Dove Comprare]} \quad \texttt{[Biglietti]} \quad \texttt{[Abbonamenti]} \quad \texttt{[Aiuto]}
 \end{tcolorbox}
 \end{center}
 ...
 Scopri tutti i canali di acquisto $\rightarrow$''
 \end{tcolorbox}
 \end{center}
...
\begin{itemize}
 ...
 \item \textbf{Profondità contenuti corporate ridotta}: Le informazioni aziendali sono accessibili dal footer, mentre oggi sono in primo piano nel menu principale.
\end{itemize}
}
\end{tcolorbox}
...
\begin{enumerate}[leftmargin=*]
 ...
 \item \textbf{Ultima priorità --- Contenuti corporate (footer)}:
 \begin{itemize}
 ...
 \end{itemize}
 Questi contenuti sono relegati al footer del sito, \textbf{mai} nel menu di navigazione principale (dove oggi occupano spazio prezioso).

 \item \textbf{Trainer-mode per intermediari (UR-05)}: Ogni guida ha una modalità ``per operatori'' che aggiunge, rispondendo allo use case \textbf{UR-05} (Guida abbonamento):
 \begin{itemize}
 ...
 \end{itemize}
 La trainer-mode è accessibile da un toggle in alto nelle guide o dal pulsante fisso \texttt{[$\bigcirc$]} in basso a destra, e persiste per l'intera sessione via \texttt{localStorage}.
...
\noindent\textit{Nota: I componenti descritti di seguito sono suddivisi tra \textbf{implementati} nel prototipo HTML/CSS/JS e \textbf{progettuali} (raccomandazioni per lo sviluppo futuro).}

\begin{enumerate}[leftmargin=*]

 \item \textbf{Persistenza della sessione per trainer-mode} (\textbf{implementato}): Il sistema salva automaticamente lo stato delle guide in modalità trainer-mode alla chiusura del browser o al timeout via \texttt{localStorage} e \texttt{sessionStorage}. Al successivo accesso (anche senza login), l'intermediario può riprendere la guida da dove aveva lasciato. Memorizza:
 \begin{itemize}
 ...
 \item \textbf{Ponte QR code tra Punti Tper e sito web (UR-04)} (\textbf{progettuale} -- non implementato nel prototipo corrente): Quando un operatore al Punto Tper completa un'assistenza, può generare un QR code stampabile che l'utente porta a casa, rispondendo allo use case \textbf{UR-04} (Ponte fisico/digitale). Inquadrandolo con lo smartphone, l'utente arriva su \texttt{tper.it} a:
 \begin{itemize}
 ...
 \item \textbf{Analytics dei canali di acquisto (anonimi)} (\textbf{progettuale}): Il sistema traccia in modo anonimo:
 \begin{itemize}
 ...
 \item \textbf{Generazione automatica del riepilogo stampabile} (\textbf{progettuale}): Alla fine di ogni guida completata (in particolare in trainer-mode), il sistema genera automaticamente:
 \begin{itemize}
 ...
 \item \textbf{Precaricamento contenuti multilingua} (\textbf{implementato} via \texttt{i18n.js}): Il sistema carica tutte le traduzioni IT/EN/AR all'avvio (circa 297 chiavi per lingua), garantendo tempi di risposta rapidi nel passaggio da una lingua all'altra. I contenuti includono:
 \begin{itemize}
...
{\small
La tabella seguente riassume le differenze architetturali tra l'attuale \texttt{tper.it} e il redesign proposto. Per un confronto visivo approfondito (screenshot Before/After), si rimanda al documento \texttt{Comparison/00-comparison-main.tex}.
...
\hline
\textbf{Natura del sito} & Misto info + transazione & \textbf{Esclusivamente informativo} \\
\hline
\textbf{Homepage} & News aziendali & \textbf{Route planner hero} + banner canali \\
\hline
\textbf{Organizzazione} & Per struttura aziendale & Per \textbf{intento utente} \\
\hline
\textbf{Etichette} & Terminologia interna & \textbf{Linguaggio utente} + icone \\
\hline
\textbf{Guida canali} & Nessuna & \textbf{``Dove Comprare''} + wizard \\
\hline
\textbf{Multilingua} & Solo italiano & \textbf{IT/EN/AR} \\
\hline
\textbf{Trainer-mode} & Inesistente & \textbf{Toggle} guide + note intermediario \\
\hline
\end{tabular}
}
\end{tcolorbox}
...
Basata sulle evidenze di Phase A (Ethnographic Assessment) e Phase B (Feasibility Study: personas Maria, Laura, Chris, Fatima; use case UR-01 ÷ UR-05)}
\end{center}
