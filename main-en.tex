...
\begin{tcolorbox}[personabox]
    ...
    \textbf{Course:} Master's Degree in Computer Science for Management
\end{tcolorbox}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
...
\noindent\textbf{Issue H1 --- Guideline G5 (Trust and Credibility)}\phantomsection\label{iss:H1}\\
...
\noindent\textbf{Issue H2 --- Guideline G8 (Search)}\phantomsection\label{iss:H2}\\
...
\noindent\textbf{Issue H3 --- Guideline G10 (Accessibility)}\phantomsection\label{iss:H3}\\
...
\noindent\textbf{Issue H4 --- Guideline G2 (Task Orientation)}\phantomsection\label{iss:H4}\\
...
\noindent\textbf{Issue H5 --- Guideline G9 (Help, Feedback, Error
...
\noindent\textbf{Issue H1 --- Violation G5 (Trust and Credibility)}\phantomsection\label{atac:H1}\\
...
\noindent\textbf{Issue H2 --- Violation G8 (Search)}\phantomsection\label{atac:H2}\\
...
\noindent\textbf{Issue H3 --- Violation G10}\phantomsection\label{atac:H3}\\
...
\noindent\textbf{Issue H4 --- Violation G2}\phantomsection\label{atac:H4}\\
...
\noindent\textbf{Issue H5 --- Violation G9}\phantomsection\label{atac:H5}\\
...
The tendency not to distinguish an interactive button from a static text string experimentally confirms the severity of \textbf{\hyperref[iss:H3]{H3}} (Visual and cognitive accessibility).

\begin{itemize}[leftmargin=*]
    \item \textbf{Affordance and visual contrast (\hyperref[iss:H3]{H3}):} Standardize buttons with clear geometric shapes and visual elevation from the background. Introduce marked \textit{hover} and \textit{focus} micro-interactions to make the distinction from static text immediate and unambiguous.
...
\medskip
\begin{enumerate}[leftmargin=*]
    \item \textbf{Task Orientation (\hyperref[iss:H4]{H4}):} Clearly signal to the user the transition from the informational site to the purchase platform, which today occurs without any notice and without explaining why the interface changes.
...
\subsection*{SUS score}
\begin{tcolorbox}[base]
\textbf{SUS score: 52.5/100}\\[0.2cm]
...
\subsection*{Outcomes and reflections useful for design}
\begin{tcolorbox}[base]
Chris Harper's data shows that a foreign user with good digital literacy can manage basic information functions and fare consultation, but encounters difficulties in the more complex flows related to discounts and online purchase. The 60\% completion rate is higher than that recorded on TPER for the same subject, but remains insufficient for satisfactory autonomous use. The failure on Tasks 4 and 5 confirms the criticality of \textbf{Issue \hyperref[atac:D3]{D3}} and \textbf{Issue \hyperref[atac:D5]{D5}}, i.e., inaccessible language and overly burdensome authentication for those who do not know the local system well. The TPER redesign must therefore offer a guest-friendly path, clearer language and a step-by-step guide for purchase, discounts and renewal. The SUS of 52.5/100 confirms a perception of usability that is only minimally acceptable, highlighting that for non-Italian users the clarity of the steps is as crucial as the quality of the information.
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*,itemsep=8pt]
    ...
    \item \textbf{\hyperref[iss:H5]{H5} --- Absence of trainer mode
    ...
    \item \textbf{\hyperref[iss:H3]{H3} --- Visual accessibility:
...
architecture, trainer mode, authentication, accessibility, error
...
\begin{tcolorbox}[base]
    ...
    \textbf{Nationality:} Moroccan, resident in Italy for 22 years
    \\[2pt]
...
\begin{tcolorbox}[base]
    \textbf{Persona description} \\[4pt]
    {\small
    \textbf{Age:} 79 years \hfill
    \textbf{Gender:} Female \hfill
    \textbf{Nationality:} Italian \\[2pt]
    ...
    \textbf{Relevant physical issues:} Presbyopia (uses reading glasses), hand arthritis that reduces fine motor skills, reduced mobility (walks with a cane for long journeys). \\[2pt]
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item \textbf{RF-12 --- Trainer/user co-navigation mode (cfr. H5):} The system must offer a shared view that allows the trainer to highlight elements, dictate steps, and guide without physically taking control of the device (e.g. pointing overlay, highlighted steps).

        \item \textbf{RF-13 --- Analog-digital bridge (cfr. H3, R6):} The user must be able to receive printable outputs (paper reminders, QR codes, deadline calendars) to maintain a link with pre-existing analog habits.
    \end{itemize}
    }
\end{tcolorbox}
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        \item \textbf{RF-14 --- Platform change notification (cfr. H4):} The transition \texttt{tper.it} $\rightarrow$ \texttt{solweb.tper.it} must be signaled with a context indicator (banner, overlay, visual note). The user must know they have changed environment and why.
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item \textbf{RF-26 --- Intent-based search engine (cfr. H2):} Internal search must support synonyms and semantic queries (``discount'', ``sconti'', ``monthly pass'') beyond exact bureaucratic terminology. For a user unfamiliar with the domain, formulating the right query must not be a prerequisite.

        \item \textbf{RF-27 --- Informative content in clear language (cfr. D2, H3):} Descriptive pages (subscriptions, discounts) must use a non-bureaucratic tone, explaining regulatory concepts (ISEE, DSU, resolutions) in language accessible even to those unfamiliar with the Italian institutional context.
    \end{itemize}
    }
\end{tcolorbox}
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        \item \textbf{RF-30 --- Semantic and intent-based search (cfr. H2):} The internal search engine must return results for ``discount'', ``sconti'', ``risparmio'' as well as ``agevolazioni''. For a user with language competence 2/5 and domain competence 2/5, knowing the exact bureaucratic term cannot be a prerequisite.
...
\begin{tcolorbox}[headerbox]
 \color{white}\LARGE\bfseries\sffamily STRUCTURE BLUEPRINT \\[0.1cm]
 \color{white!80}\small\sffamily Phase C --- Design Proposal
\end{tcolorbox}
...
\begin{tcolorbox}[headerbox]
 \color{white}\LARGE\bfseries\sffamily WIREFRAME \\[0.1cm]
 \color{white!80}\small\sffamily Phase C --- Design Proposal
\end{tcolorbox}
...
% ================================================================
% WF7: SOLWEB PAYMENT AND CONFIRMATION
% ================================================================
\section{Wireframe 7: solweb --- Payment and Confirmation}
\label{wf7}
...
\caption{solweb --- Payment with cart summary (top) and post-purchase confirmation screen (bottom)}
\label{fig:wf7}
\end{figure}
...
\hline
WF1 & Homepage Guest & Route planner, 6 ``What to do?'' cards, Dove Comprare sidebar & Nav tabs, hero search, channels in sidebar, news \\
\hline
WF2 & Pianifica & Route search form, timetable/duration/price results, map/list toggle & Core function, Dove Comprare link per result, info prices \\
\hline
WF3 & Dove Comprare & 5 channel cards: solweb, Roger, Punti Tper, Tabaccherie, Contactless; comparison table & Cards with CTA, comparison by need \\
\hline
WF4 & Biglietti & Ticket types with prices and validity, recommended badge, channel legend & Informational, channel icons per type, updated prices \\
\hline
WF5 & Aiuto + Trainer-Mode & FAQ by channel, guides section, trainer-mode toggle, 5-step guide & ON/OFF toggle, step-by-step visible, channel indicated \\
\hline
...
\hline
WF7 & solweb: Payment + Confirmation & Cart summary, payment, confirmation, next options & Transactional flow, purchased ticket, print CTA \\
\hline
\end{tabular}
...
\input{Comparison/00-comparison-body-en.tex}
...
& Early May 2026
& Original TPER website assessment \\
\hline
\textbf{V1} 
& \texttt{Website - Inter/}
& June 22, 2026, 12:00
& \parbox{4.5cm}{\raggedright --- Heuristic evaluation (D1--D5, H1--H5) \\ --- Formative test (Phase A) \\ --- 4 personas (Phase B)} \\
\hline
\textbf{Final (v2.0)} 
& \texttt{Website - Finale/} (final version)
& July 3, 2026, 10:00
& \parbox{3.8cm}{\raggedright --- Heuristic evaluation (Post-Fix) \\ --- Formative Test (2 subjects) \\ --- Summative Test (2 subjects)} \\
\hline
\end{tabular}
...
& WF1 (Homepage: ``What do you want to do?'' grid and channel sidebar) \newline BP1 (IA: purchase vs informational sections) \newline BP3 (User Journey: decision point ``where to buy?'') \\
\hline
...
\begin{tcolorbox}[summarybox]
\small
\textbf{Note:} Each issue is related to the requirements from Phase A (R1--R13), Phase B (Cast Card --- Use Case UR-01--UR-05) and Phase D (Cognitive Walkthrough CW-01--CW-11, Formative Test FT1--FT7). The design solutions can be located in the wireframes (WF1--WF7) and blueprints (BP1--BP3).
\end{tcolorbox}
...
\input{FaseD2/02-inspection-card-body-en.tex}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Informational red banner on ``Abbonamenti'' (both):}
  The banner ``Tper is an informational site, to buy or renew subscriptions visit the Dove Comprare section'' reduced attempts to purchase on the wrong page. Both subjects noticed the banner and followed the link.
...
\subsection*{SUS score}
\begin{tcolorbox}[base]
\textbf{SUS Score: 70.0/100}\\[0.2cm]
\small Score well above the acceptability threshold
(50/100) and above 68/100, a value indicating \textbf{good
usability} according to the standardized SUS scale. The data marks
an improvement of +42.5 points compared to the Phase A SUS on the original
...
from 27.5/100 to 70.0/100, exceeding the \textbf{good usability} threshold
(68/100).
...
to Laura (27.5 vs 37.5 in Phase A) to only 2.5 points (70.0 vs 72.5)
...
Maria Borriello & 27.5/100 & 70.0/100 \\
\midrule
\textbf{Average} & \textbf{32.5/100} & \textbf{71.25/100} \\
\textbf{Variance} & 50.0 & 3.13 \\
\bottomrule
\end{tabular}
\end{center}
...
The variance drops drastically from 50.0 (Phase A) to 3.13
...
average improvement of +38.75 SUS points.
...
\begin{itemize}[leftmargin=*]
  ...
  71.25/100 (good usability), with both subjects above
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Wireframe:} 7 screens (WF1--WF7) covering the entire tper.it + solweb ecosystem.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Summative Test:} 2 subjects (same as Phase A), average SUS \textbf{71.25/100} (good usability), completion rate 80\%, error reduction --73\%, variance reduced from 50.0 to 3.13.
\end{itemize}
...
\midrule
Average SUS & 32.5/100 & \textbf{71.25/100} \\
Tasks completed & 1/10 (10\%) & \textbf{8/10 (80\%)} \\
SUS variance & 50.0 & \textbf{3.13} \\
