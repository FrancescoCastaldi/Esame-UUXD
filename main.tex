...
\begin{tcolorbox}[colback=primaryColor,colframe=primaryColor,arc=1.5mm,halign=center,valign=center,height=2.5cm]
    \color{white}\LARGE\bfseries\sffamily TperTutti \\[0.2cm]
...
% ================================================================
% TEAM CARD — Valida per tutto il progetto
% ================================================================
\section*{Team Card}
...
\small
\textbf{Nome del Team}: TperTutti \\[6pt]
Il presente documento costituisce il progetto di redesign dell'ecosistema digitale TPER S.p.A., 
sviluppato nell'ambito del corso di \textit{Progettazione UUXD} dell'Università di Bologna, 
A.A.\ 2025/2026. La Team Card è unica e valida per tutte le fasi del progetto (Phase A, B, C, D).
\end{tcolorbox}
...
\begin{tcolorbox}[personabox]
    \textbf{Membro 1} \\[4pt]
    \textbf{Nome:} Francesco Castaldi \\[2pt]
    \textbf{Matricola:} 0001230662 \\[2pt]
    \textbf{Email:} francesco.castaldi4@studio.unibo.it \\[2pt]
    \textbf{Corso:} Laurea Magistrale in Informatica per il Management
\end{tcolorbox}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\begin{tcolorbox}[personabox]
    \textbf{Membro 2} \\[4pt]
    \textbf{Nome:} Stefano Mercurio \\[2pt]
    \textbf{Matricola:} 1237578 \\[2pt]
...
transazionale (\texttt{solweb}\discretionary{-}{-}{.}\texttt{tper.it}): acquisto e rinnovo di abbonamenti
...
\begin{itemize}
    ...
    \item Nessuna modalità di affianca\-mento caregiver: il sito non prevede vista condivisa, sessione co-navigabile o funzioni trainer.
...
\noindent\textit{Analisi condotta secondo le linee guida UserFocus. Per ogni issue viene indicata la linea guida violata, la localizzazione nel sistema.}\\[2pt]
...
\noindent\textbf{Issue R1 — Linea Guida G5 (Trust and Credibility)}\phantomsection\label{iss:H1}\\
...
\noindent\textbf{Issue R2 — Linea Guida G8 (Search)}\phantomsection\label{iss:H2}\\
...
\noindent\textbf{Issue R3 — Linea Guida G10 (Accessibility)}\phantomsection\label{iss:H3}\\
...
\noindent\textbf{Issue R4 — Linea Guida G2 (Task Orientation)}\phantomsection\label{iss:H4}\\
...
Tolerance)}\phantomsection\label{iss:H5}\\
...
\noindent\textit{Analisi condotta secondo le linee guida UserFocus. Per ogni problema viene indicata la linea guida violata, la localizzazione nel sistema e la classificazione della gravità.}\\[2pt]
\raggedright\url{http://www.userfocus.co.uk/resources/guidelines.html}\par
...
\noindent\textbf{Issue R1 — Violazione G5 (Fiducia e credibilità)}\phantomsection\label{atac:H1}\\
...
\noindent\textbf{Issue R2 — Violazione G8 (Ricerca)}\phantomsection\label{atac:H2}\\
...
\noindent\textbf{Issue R3 — Violazione G10}\phantomsection\label{atac:H3}\\
...
\noindent\textbf{Issue R4 — Violazione G2}\phantomsection\label{atac:H4}\\
...
\noindent\textbf{Issue R5 — Violazione G9}\phantomsection\label{atac:H5}\\
...
La tendenza a non distinguere un pulsante interattivo da una stringa di testo statico conferma sperimentalmente la gravità della \textbf{\hyperref[iss:H3]{H3}} (Accessibilità visiva e cognitiva). 

\begin{itemize}[leftmargin=*]
    \item \textbf{Affordance e contrasto visivo (\hyperref[iss:H3]{H3}):} Standardizzare i pulsanti con forme geometriche chiare ed elevazione visiva rispetto allo sfondo. Introdurre micro-interazioni di \textit{hover} e \textit{focus} marcate per rendere immediata e inequivocabile la distinzione dal testo statico.
...
\medskip
\begin{enumerate}[leftmargin=*]
    \item \textbf{Task Orientation (\hyperref[iss:H4]{H4}):} Segnalare chiaramente all'utente il passaggio dal sito informativo alla piattaforma di acquisto, che oggi avviene senza alcun preavviso e senza spiegare perché l'interfaccia cambi.
...
\subsection*{Modality of recruitment of subjects}
\begin{tcolorbox}[base]
Medesima modalità di reclutamento descritta nel test sul sito TPER
(Sezione~\ref{sec:recruitment-modality-client}). I soggetti sono
stati ricontattati direttamente per concordare una seconda sessione
di testing.
\end{tcolorbox}
...
\subsection*{Motivation for choice of subjects}
\begin{tcolorbox}[base]
Stessa motivazione del test sul sito TPER
(Sezione~\ref{sec:recruitment-motivation-client}): i soggetti
sono stati scelti per la loro rappresentatività dei segmenti
S1 (anziano pendolare), S2 (lavoratore straniero) e S3
(cittadino allo sportello).
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*,itemsep=8pt]
    ...
    \item \textbf{\hyperref[iss:H5]{H5} --- Assenza di modalità trainer 
    ...
    \item \textbf{\hyperref[iss:H3]{H3} --- Accessibilità visiva: 
    ...
    \item \textbf{\hyperref[iss:H4]{H4} --- Task orientation:
    ...
    \item \textbf{\hyperref[iss:H1]{H1} --- Trust and credibility: 
    ...
    \item \textbf{\hyperref[iss:H2]{H2} --- Search: 
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item \textbf{RF-12 --- Modalità co-navigazione trainer/utente (cfr. H5):} Il sistema deve offrire una vista condivisa che permetta al trainer di evidenziare elementi, dettare passaggi e guidare senza prendere fisicamente il controllo del dispositivo (es. overlay di puntamento, step evidenziati).

        \item \textbf{RF-13 --- Ponte analogico-digitale (cfr. H3, R6):} L'utente deve poter ricevere output stampabili (promemoria cartacei, QR code, calendario scadenze) per mantenere un aggancio con le abitudini analogiche preesistenti.
    \end{itemize}
    }
\end{tcolorbox}
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        \item \textbf{RF-14 --- Segnalazione cambio piattaforma (cfr. H4):} Il passaggio \texttt{tper.it} $\rightarrow$ \texttt{solweb.tper.it} deve essere segnalato con un indicatore di contesto (banner, overlay, nota visiva). L'utente deve sapere di aver cambiato ambiente e perché.
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item \textbf{RF-26 --- Motore di ricerca per intenti (cfr. H2):} La ricerca interna deve supportare sinonimi e query semantiche (``discount'', ``sconti'', ``monthly pass'') oltre alla terminologia burocratica esatta. Per un utente che non conosce il dominio, formulare la query giusta non deve essere un prerequisito.

        \item \textbf{RF-27 --- Contenuti informativi in lingua chiara (cfr. D2, H3):} Le pagine descrittive (abbonamenti, agevolazioni) devono utilizzare un tono non burocratico, spiegando i concetti normativi (ISEE, DSU, delibere) in linguaggio accessibile anche a chi non ha familiarità con il contesto istituzionale italiano.
    \end{itemize}
    }
\end{tcolorbox}
...
\begin{tcolorbox}[base]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item \textbf{RF-29 --- Mappatura esplicita dell'ecosistema multipiattaforma (cfr. D5, H4):} Il sistema deve dichiarare esplicitamente le relazioni tra piattaforme (solweb = acquisto, Mi Muovo = supporto fisico, app TPER = digitale) al momento opportuno, non a posteriori. L'utente deve sapere su quale piattaforma sta operando e cosa succede dopo.
    ...
    \begin{itemize}[leftmargin=*]
        \item \textbf{RF-30 --- Ricerca semantica e per intenti (cfr. H2):} Il motore di ricerca interno deve restituire risultati per ``discount'', ``sconti'', ``risparmio'' oltre che per ``agevolazioni''. Per un utente con competenza linguistica 2/5 e competenza di dominio 2/5, conoscere il termine burocratico esatto non può essere un prerequisito.
...
\hline
\texttt{solweb}\discretionary{-}{-}{.}\texttt{tper.it} & Digitale & Abbonamenti online, rinnovo, ricarica, agevolazioni ISEE & Browser web, richiede account \\
\hline
...
% ================================================================
% WF6: ABBONAMENTI + TRAINER (RINNOVO)
% ================================================================
\section{Wireframe 6: Abbonamenti --- Trainer Rinnovo}
\label{wf6}

\begin{figure}[h!]
\centering
\wfgraphic{Wireframes/screenshot-wf6.png}
\caption{Abbonamenti --- Tipologie e trainer-mode Rinnovo attivo con highlight}
\label{fig:wf6}
\end{figure}

% ================================================================
% WF7: SOLWEB PAGAMENTO E CONFERMA
% ================================================================
\section{Wireframe 7: solweb --- Flusso di Pagamento e Conferma}
\label{wf7}

\begin{figure}[h!]
\centering
\includegraphics[width=0.66\textwidth,keepaspectratio]{Wireframes/screenshot-wf7a.png}
\vspace{0.15cm}
\includegraphics[width=0.66\textwidth,keepaspectratio]{Wireframes/screenshot-wf7b.png}
\caption{solweb --- Pagamento con riepilogo carrello (sopra) e schermata di conferma post-acquisto (sotto)}
\label{fig:wf7}
\end{figure}
...
% SUMMARY TABLE
% ================================================================
\subsection*{Riepilogo dei Wireframe}

\begin{table}[H]
\centering
\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|p{1.8cm}|p{2.8cm}|p{5.2cm}|p{5.2cm}|}
\hline
\textbf{Wireframe} & \textbf{Schermata} & \textbf{Contenuto principale} & \textbf{Elementi chiave} \\
\hline
WF1 & Homepage Guest & Route planner, 6 card ``Cosa vuoi fare?'', sidebar Dove Comprare & Nav tabs, hero search, canali in sidebar, news \\
\hline
WF2 & Pianifica & Form ricerca percorso, risultati orari/durata/prezzo, toggle mappa/elenco & Core function, link Dove Comprare per risultato, prezzi informativi \\
\hline
WF3 & Dove Comprare & 5 schede canale: solweb, Roger, Punti Tper, Tabaccherie, Contactless; tabella comparativa & Card con CTA, confronto per esigenza \\
\hline
WF4 & Biglietti & Tipologie biglietto con prezzi e validit\`a, badge consigliato, legenda canali & Informativo puro, icone canale, prezzi aggiornati \\
\hline
WF5 & Aiuto + Trainer-Mode & FAQ per canale, sezione guide, toggle trainer-mode, guida 5 passi & Toggle ON/OFF, passo-passo visibile, canale indicato \\
\hline
WF6 & Abbonamenti & Tipologie abbonamento, badge consigliato, trainer-mode Rinnovo attivo & Trainer highlight, confronto piani, CTA per canale \\
\hline
WF7 & solweb: Pagamento + Conferma & Riepilogo carrello, pagamento, conferma acquisto, opzioni successive & Flusso transazionale, biglietto acquistato, CTA stampa \\
\hline
\end{tabular}
\caption{Riepilogo dei wireframe prodotti (tper.it + solweb.tper.it)}
\label{tab:wfsum}
\end{table}
...
\input{Comparison/00-comparison-body.tex}
...
\hline
\textbf{V0} 
& \texttt{Tper sito originale/}
& Inizio Maggio 2026
& Sito TPER originale --- valutazione preliminare \\
\hline
\textbf{V1} 
& \texttt{Website - Inter/}
& 22 Giugno 2026, ore 12:00
& \parbox{4.5cm}{\raggedright --- Valutazione euristica (D1--D5, H1--H5) \\ --- Formative test (Phase A) \\ --- 4 personas (Phase B)} \\
\hline
\textbf{Finale (v2.0)} 
& \texttt{Website - Finale/} (versione finale)
& 3 Luglio 2026, ore 10:00
& \parbox{3.8cm}{\raggedright --- Valutazione euristica (Post-Fix) \\ --- Formative Test (2 soggetti) \\ --- Summative Test (2 soggetti)} \\
\hline
\end{tabular}
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{|p{0.8cm}|p{3.2cm}|p{3.3cm}|p{6.2cm}|}
\hline
...
\begin{tcolorbox}[summarybox]
\small
\textbf{Nota:} Ogni issue \`e correlata ai requirement emersi dalla Phase A (R1--R13), dalla Phase B (Cast Card --- Use Case UR-01--UR-05) e dalla Phase D (Cognitive Walkthrough CW-01--CW-11, Formative Test FT1--FT7). Le soluzioni progettuali sono localizzabili nei wireframe (WF1--WF7) e nei blueprint (BP1--BP3).
\end{tcolorbox}
...
\input{FaseD2/02-inspection-card-body.tex}
...
\noindent\textbf{Varianza:} 12,5 --- range contenuto, a
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Banner rosso informativo su ``Abbonamenti'' (entrambi):}
  Il banner ``Tper è un sito informativo, per acquistare o rinnovare abbonamenti visita la sezione Dove Comprare'' ha ridotto i tentativi di acquisto sulla pagina sbagliata. Entrambi i soggetti hanno notato il banner e seguito il link.
...
\begin{itemize}[leftmargin=*]
  ...
  652 s a 164,8 s per Maria; da 183,8 s a 103,0 s per Laura).
...
% ================================================================
% RECAP FINALE DEL PROGETTO
% ================================================================

\newpage
\begin{center}
  {\color{primaryColor}\LARGE\bfseries\sffamily Recap Finale del Progetto}\\[0.3cm]
  \color{borderColor}\hrule
\end{center}
...
\noindent\textbf{Phase A --- Ricerca e Analisi}
\begin{itemize}[leftmargin=*]
  \item \textbf{Client:} TPER S.p.A. --- Ecosistema digitale complesso (6 canali), barriere cognitive, linguistiche e terminologiche per l'utenza fragile.
  \item \textbf{Segmentazione:} 3 combinazioni prioritarie --- S1 (anziana pendolare), S2 (lavoratore straniero), S3 (cittadino allo sportello).
  \item \textbf{Assessment:} 10 issue critiche sul sito TPER (Direct + Reverse Analysis), 10 sul competitor ATAC.
  \item \textbf{User Testing:}   4 soggetti, SUS medio 37,5/100 (inaccettabile), 6/20 task completati (30\%).
  \item \textbf{Raccomandazioni:} 13 raccomandazioni progettuali (R1--R13) suddivise in 6 macro-aree.
\end{itemize}

\vspace{0.2cm}

\noindent\textbf{Phase B --- Personas e Cast}
\begin{itemize}[leftmargin=*]
  \item \textbf{Protagonista:} Fatima Benali (68 anni, marocchina, analfabeta in italiano scritto) --- icona del design inclusivo.
  \item \textbf{Cast:} 4 personas (Fatima, Liliana, David, Roberto) con profili C\&A, 5 use case narrati.
  \item \textbf{Requisiti:} 43 requirement funzionali (RF-01\ldots{}RF-43) + 10 requisiti di cast (PR-01\ldots{}PR-05, UR-01\ldots{}UR-05).
\end{itemize}

\vspace{0.2cm}

\noindent\textbf{Phase C --- Progettazione}
\begin{itemize}[leftmargin=*]
  \item \textbf{IA:} Approccio misto top-down/bottom-up, riorganizzazione etichette, 5 campi metadati, 9 ausili alla navigazione.
  \item \textbf{Blueprint:} 3 diagrammi (IA tree, ecosistema multi-canale, user journey con trainer-mode).
  \item \textbf{Wireframe:} 7 schermate (WF1--WF7) che coprono l'intero ecosistema tper.it + solweb.
  \item \textbf{Comparison:} 5 confronti Before/After con 16 differenze strutturali.
  \item \textbf{Design Summary:} 10 issue risolte, tracciabili dai requisiti Phase A/B ai test Phase D.
\end{itemize}

\vspace{0.2cm}

\noindent\textbf{Phase D --- Validazione}
\begin{itemize}[leftmargin=*]
  \item \textbf{Cognitive Walkthrough:} 11 issue (CW-01\ldots{}CW-11) su 5 task, 9 raccomandazioni implementate in v2.0.
  \item \textbf{Formative Test:} 2 soggetti, SUS medio 57,5/100, 90\% completamento, urgency curve FT1--FT7.
  \item \textbf{Summative Test:} 2 soggetti (stessi della Phase A), SUS medio \textbf{71,75/100} (buona usabilità), tasso di completamento 80\%, riduzione errori --73\%, varianza ridotta da 50,0 a 1,13.
\end{itemize}
...
\begin{tabular}{lcc}
\toprule
\textbf{Metrica chiave} & \textbf{Phase A} & \textbf{Phase D (v2.0)} \\
\midrule
SUS medio & 32,5/100 & \textbf{71,75/100} \\
Task completati & 1/10 (10\%) & \textbf{8/10 (80\%)} \\
Varianza SUS & 50,0 & \textbf{1,13} \\
Tempo medio task & 417,9 s & \textbf{133,9 s} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.2cm}
\small Nota: i valori Phase A si riferiscono al subset dei 2 soggetti ritestati (Laura Palumbo e Maria Borriello), non alla media dei 4 soggetti originali.

\vspace{0.2cm}

\noindent Il progetto \textbf{TperTutti} ha ridisegnato l'esperienza digitale TPER \textbf{mettendo al centro l'utente fragile}: anziani, stranieri, cittadini con bassa competenza digitale. I risultati quantitativi e qualitativi confermano che un design inclusivo e guidato produce benefici misurabili per tutti, e benefici proporzionalmente maggiori per chi parte da condizioni più svantaggiate.
