...
In the original site, three pages overlapped, generating confusion and duplication:

\begin{itemize}[nosep]
    \item \textbf{Tickets}: listed tickets + sales channels + tariffs --- too many functions for one page;
    \item \textbf{Subscriptions}: listed subscriptions + channels + tariffs --- the same duplication as the Tickets page;
    \item \textbf{Tariffs}: a separate pricing page, \textbf{redundant} and disconnected from the usage context.
\end{itemize}

Result: the user had to \textbf{jump between 3 pages} to answer questions like \textit{``which subscription should I buy?''} and \textit{``where do I buy it?''}. The channel guide (``Sales Network'') was buried three navigation levels deep.
...
{\footnotesize 3 overlapping pages: duplicated content, buried channels, corporate terminology}
...
\cellcolor{primaryColor!15} &
\textit{3 overlapping pages} &
\textit{3 specialised pages} \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Structure}}} &
3 pages with duplicated content and blurred boundaries. Tickets and Subscriptions both contain references to tariffs and channels. The Tariffs page is redundant. 3+ level navigation to reach the channel guide (D2). &
...
\textbf{Channels}: ``Sales Network'' buried under Tickets and Subscriptions, reachable in 3 clicks. \textbf{Tariffs}: separate pricing page, disconnected from the usage context --- the user must remember the price while navigating elsewhere to decide (D3). &
...
% ============================================================
% COMPARISON 4: solweb PURCHASE FLOW
% ============================================================
\section{\texttt{solweb.tper.it} Purchase Flow}
\label{sec:comparison-acquisto}

\textbf{D4+D5 $\rightarrow$ R7+R10} --- No error recovery $\rightarrow$ Guided messages + guest path. Chris 498s, 8 errors, 10 backtracks on T3.
...
\par\small\textit{Before: original purchase flow --- mandatory registration, no guidance}
\end{minipage}
\hfill
\begin{minipage}[t]{0.498\textwidth}
\centering
\includegraphics[width=\textwidth]{Redesign Tper 2.0/RiepilogoS.png}
\par\small\textit{After: purchase flow 2.0 --- progress bar, guided form, order summary}
\end{minipage}
\caption{\texttt{solweb.tper.it} Purchase Flow}
...
& \textbf{Before} & \textbf{After} \\ \hline
\cellcolor{primaryColor!15} &
\textit{Original solweb purchase flow} &
\textit{solweb 2.0 purchase flow} \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Progress}}} &
No progress indicator. The user does not know where they are in the process or how many steps remain. Frequent abandonment due to disorientation (Issue D2). &
\textbf{4-step progress bar}: Login $\rightarrow$ Choose Plan $\rightarrow$ Documents $\rightarrow$ Payment. Each step shows: step number, text description, graphical completion indicator (green = completed, red = active). Visible on all flow pages (R3). \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Documents}}} &
No preventive checklist. The user discovers which documents are needed only after starting the flow, often causing mid-flow abandonment. Elderly users forced to return to the counter because they lack the required documents (R11). &
\textbf{Interactive checklist} with progress bar (0/4 $\rightarrow$ 4/4). 4 document cards with visual reproduction of the actual document (where the number is, where the expiry date is). ``I have this document'' checkbox. Guided upload. ``Required Documents'' badge visible before starting (R11). \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Payment}}} &
Payment form without order summary. No security icons. Generic error messages: ``Error --- try again''. The user does not know what went wrong or how to fix it (Issue D4). &
\textbf{Order summary} always visible: plan, duration, total. 3 payment methods with radio buttons (Card, PayPal, Bank Transfer). Terms checkbox. Exponential error messages: what happened + why + how to fix + alternative (R10). \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Flexibility}}} &
Single possible path: registration $\rightarrow$ login $\rightarrow$ purchase. No variant for top-up, impersonal renewal, or fines (Issue D5). &
\textbf{Multi-flow}: pagamento.html handles 4 different flows (purchase, renewal, top-up, fines) with adaptive breadcrumbs and forms. The guest path for impersonal renewal and top-up completely skips registration (R7). \\ \hline
\end{tabular}
\caption{solweb Purchase Flow}
...
% ============================================================
% COMPARISON 5: POST-PURCHASE SUMMARY
% ============================================================
\section{\texttt{solweb.tper.it} Post-Purchase Summary}
\label{sec:comparison-riepilogo}

\textbf{UR-02 $\rightarrow$ R6} --- PDF mistaken for valid ticket $\rightarrow$ 3 guided post-purchase steps. Risk of onboard fine.
...
\par\small\textit{Before: original summary --- confirmation only, no post-purchase guidance}
\end{minipage}
\hfill
\begin{minipage}[t]{0.498\textwidth}
\centering
\includegraphics[width=\textwidth]{Redesign Tper 2.0/AcquistoS.png}
\par\small\textit{After: confirmation 2.0 --- 3 guided steps + printable receipt}
\end{minipage}
\caption{Post-Purchase Summary}
...
& \textbf{Before} & \textbf{After} \\ \hline
\cellcolor{primaryColor!15} &
\textit{Original post-purchase summary} &
\textit{Post-purchase confirmation in solweb 2.0} \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Confirmation}}} &
Bare confirmation message. No explanation of what happens next. The generated PDF is mistaken for a valid travel document --- but it is not (UR-02). &
\textbf{H1}: ``Thank you for your purchase!''. Complete progress bar (4/4 with green checkmark). Detailed order summary: plan, email, order number, payment method, total. ``Download Receipt (PDF)'' button (R6). \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Instructions}}} &
\textbf{Absent.} No guidance on how to use the purchased ticket/subscription. The user does not know whether to print something, show their phone, or validate a card (UR-02). &
\textbf{3 guided post-purchase steps}: (1) You will receive a confirmation email with your order details, (2) Your card will activate automatically within 24 hours, (3) Show your card to the inspector onboard. Each step is numbered and illustrated with icons (R6, UR-02). \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Support}}} &
No post-purchase support link. A user in difficulty must independently search for customer service. &
Help link always visible. Order number reference for assistance. Option to return to the dashboard or home. \\ \hline

\raisebox{0.5cm}{\rotatebox{90}{\textbf{Cross-channel}}} &
No connection with other channels. The journey ends without guidance on how to use the purchased service on Roger, at Punti Tper, or onboard. &
Cross-channel references: if you purchased a subscription, we explain how to also use it on the Roger App. If you paid a fine, we show how to check the status (UR-02, R6). \\ \hline
\end{tabular}
\caption{Post-Purchase Summary}
...
\textbf{Integrated sidebar} on all 22 pages of the site. 5 trainer modules: Renew, Purchase, Trip, Ticket, Top-up. Each module automatically loads the correct page with highlights on relevant elements. Activation: on request ($\mathcal{T}$ button) or automatic after 2 consecutive user errors (R4, R5). \\ \hline
...
``Skip to main content'' link present. ARIA roles absent or partial on most interactive elements. &
\textbf{Complete ARIA}: \texttt{role="banner"}, \texttt{role="navigation"}, \texttt{role="main"}, \texttt{role="complementary"} (trainer bar), \texttt{role="contentinfo"} (footer), \texttt{role="dialog"} (modals and search overlay), \texttt{role="search"} (route planner), \texttt{aria-label} on all interactive elements, \texttt{aria-current="page"} on the active page, \texttt{aria-expanded} on hamburger and accordion, \texttt{aria-modal="true"} on overlays and modals (R8). \\ \hline
...
% SUMMARY
% ============================================================
\section{Summary of Key Differences}
...
\hline
\textbf{Dimension} & \textbf{Before (\texttt{tper.it} live)} & \textbf{After (tper 2.0)} \\ \hline
\texttt{tper.it} Homepage & Corporate news and institutional slider (D1) & Hero route planner + 6 task-oriented card grid (R1) \\ \hline
\texttt{solweb} Homepage & Login only, no guest path (D5) & 4 quick actions without registration + progress bar (R7) \\ \hline
Information Architecture & 3 overlapping pages (Tickets, Subscriptions, Tariffs), channels buried 3 levels deep (D2) & 3 specialised pages: Tickets, Subscriptions, Where to Buy --- one page, one task (R2) \\ \hline
Navigation & Mega-menu 200+ items, 5 levels deep & Flat nav 8 items + breadcrumb on all pages \\ \hline
``Where to Buy'' & Non-existent --- \textit{Sales Network} buried & Standalone section + 3-question wizard + comparison table 5$\times$7 \\ \hline
solweb / Roger & Never mentioned on homepage & Dedicated cards with direct CTAs and transition modal \\ \hline
Channel Guide & Absent --- user must deduce & Interactive wizards on 3 pages + comparison table \\ \hline
Purchase Flow & No progress, no document checklist & 4-step bar + interactive checklist (R3, R11) \\ \hline
Post-Purchase & No instructions, PDF mistaken for ticket (UR-02) & 3 guided steps + printable receipt (R6) \\ \hline
Trainer Mode & Non-existent (R5) & Sidebar on 22 pages + 5 modules + persistence + summary (R4, R5, R6) \\ \hline
FAQ / Guides & Help desk only & Channel accordion + 3 step-by-step guides + operator script \\ \hline
Language & Italian only & IT + EN + AR, 1,600+ i18n keys, RTL support (R12) \\ \hline
Font / Contrast & 10--11pt, below WCAG AA & 14pt min, 4.5:1, A+ toggle, contrast toggle (R8) \\ \hline
ARIA / Touch Targets & Partial, targets $<$ 44px & Complete: roles, labels, states; all targets $\ge$ 44$\times$44px (R8) \\ \hline
Tone & Bureaucratic/institutional & Encouraging/inclusive + micro-reinforcements (R9) \\ \hline
Terminology & Corporate (Sales Network, MiMuovo, Issuing Machines) & User-centric with glosses (Where to Buy, Online Subscriptions) \\ \hline
Errors & Generic ``Error --- try again'' messages & Exponential messages: cause + solution + alternative (R10) \\ \hline
Info Message & Absent & ``tper.it is an informational website'' banner on every page (R1) \\ \hline
Documents & No preventive checklist & Interactive checklist with visual reproduction (R11) \\ \hline
Guest Path & Non-existent --- login required for everything & Impersonal renewal, top-up, fines without registration (R7) \\ \hline
\end{tabular}
}
\end{tcolorbox}
...
Document compiled based on direct analysis of the live \texttt{tper.it} and \texttt{solweb.tper.it} sites, \\
R1--R13 recommendations from Phase B, and Phase A user testing results (4 subjects, SUS 37.5/100) \\[4pt]
