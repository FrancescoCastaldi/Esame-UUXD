  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadTrainerModules() {
      ...
      try {
        ...
        var jsonPath = (window.location.pathname.indexOf('/solweb/') !== -1 ? '../' : '') + 'js/trainer-modules.json';
        xhr.open('GET', jsonPath, false); // synchronous fallback
    ...
    /* ---------- Cross-directory path helper ---------- */
    var TRAINER_ROOT_PAGES = ['index.html', 'pianifica.html', 'orari.html', 'notizie.html',
      'dove-comprare.html', 'biglietti.html', 'abbonamenti.html', 'aiuto.html',
      'agevolazioni.html', '404.html'];

    function resolveTrainerPath(path) {
      var inSolweb = window.location.pathname.indexOf('/solweb/') !== -1;
      var hash = '';
      var hashIdx = path.indexOf('#');
      if (hashIdx !== -1) { hash = path.substring(hashIdx); path = path.substring(0, hashIdx); }
      var basePath = path.split('?')[0];
      var hasSolwebPrefix = path.indexOf('solweb/') === 0;
      var isRootPage = TRAINER_ROOT_PAGES.indexOf(basePath) !== -1;

      if (inSolweb) {
        if (hasSolwebPrefix) { path = path.substring(7); }
        else if (isRootPage) { path = '../' + path; }
      } else {
        if (!hasSolwebPrefix && !isRootPage) { path = 'solweb/' + path; }
      }
      return path + hash;
    }
    ...
    function saveTrainerState() {
      if (currentModule) {
        ...
        sessionStorage.setItem('tper-trainer-current', state);
        // Also persist to localStorage as fallback (cross-session)
        try { localStorage.setItem('tper-trainer-saved', state); } catch(e) {}
      } else {
    ...
    function restoreTrainerState() {
      try {
        // 1) Try sessionStorage first (same-tab, most accurate)
        ...
        // 2) Fallback to localStorage (cross-tab recovery)
        var ls = localStorage.getItem('tper-trainer-saved');
        if (ls) {
          var lst = JSON.parse(ls);
          currentModule = lst.module; currentStep = lst.step || 0;
          // Re-save to sessionStorage for consistency
          sessionStorage.setItem('tper-trainer-current', ls);
          return true;
        }
      } catch(e) {}
      return false;
    ...
    function renderTrainerModules() {
      ...
      trainerSteps.querySelectorAll('.trainer-module-card').forEach(function(btn) {
        btn.addEventListener('click', function() {
          ...
          // Redirect immediato alla pagina target del modulo (via resolveTrainerPath per supporto cross-directory)
          if (currentModule === 'viaggio' && cp !== 'pianifica.html') {
            saveTrainerState(); setTrainerActive(true);
            window.location.href = resolveTrainerPath('pianifica.html'); return;
          }
          if (currentModule === 'rinnovo' && cp !== 'abbonamenti.html') {
            saveTrainerState(); setTrainerActive(true);
            window.location.href = resolveTrainerPath('abbonamenti.html'); return;
          }
          if (currentModule === 'biglietto' && cp !== 'biglietti.html') {
            saveTrainerState(); setTrainerActive(true);
            window.location.href = resolveTrainerPath('biglietti.html'); return;
          }
          if (currentModule === 'ricarica' && cp !== 'dove-comprare.html') {
            saveTrainerState(); setTrainerActive(true);
            window.location.href = resolveTrainerPath('dove-comprare.html'); return;
          }
          if (currentModule === 'acquista' && cp !== 'login.html') {
            saveTrainerState(); setTrainerActive(true);
            window.location.href = resolveTrainerPath('login.html'); return;
          }
    ...
    function renderTrainerSteps() {
      ...
      if (ab && !ab.disabled) {
        ab.addEventListener('click', function() {
          ...
          if (samePage) {
            ...
            saveTrainerState();
            setTrainerActive(true);
            window.location.href = resolveTrainerPath(url);
          }
        });
      }
      ...
      if (step.highlight) {
        ...
        if (targetEl && targetEl.style.display === 'none') { targetEl.style.display = ''; }
      ...
      ab.addEventListener('click', function() {
        ...
        setTimeout(function() {
          ...
          if (container) {
            ...
            var scrollTarget = rect.top + window.pageYOffset - 20;
    ...
    function openTrainerMode() {
      ...
      if (isOpening) {
        ...
        sessionStorage.removeItem('tper-trainer-current');
        try { localStorage.removeItem('tper-trainer-saved'); } catch(e) {}
        clearHighlight();
    ...
    /* ============================
        12b. MAPPA BOLOGNA — Lightbox click
        ============================ */
    ...
    if (mapImg && mapLightbox) {
      mapImg.addEventListener('click', function() {
        mapLightbox.style.display = 'flex';
      });
      mapLightbox.addEventListener('click', function(e) {
        if (e.target === mapLightbox || e.target.tagName === 'BUTTON') {
          mapLightbox.style.display = 'none';
        }
      });
      document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && mapLightbox.style.display === 'flex') {
          mapLightbox.style.display = 'none';
        }
      });
    }
